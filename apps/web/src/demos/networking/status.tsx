/**
 * PAGE DEMO — reference example.
 *
 * Page demos wrap their body in <DemoPage> for the standard chrome:
 *   - back / overview links
 *   - title + blurb header
 *   - star (favorite) button
 *
 * Fullscreen demos (3D scenes, immersive experiences) can pass
 * hideChrome=true and render their own UI — but they still appear under
 * /d/<id> and remain favorite-able via in-demo star buttons.
 */
...
export default function StatusDemo() {
  ...
  return (
    <DemoPage
      id="status"
      title="Live clients"
      blurb="Realtime list of every websocket-connected client. Open this page in another tab — it should appear here."
      maxWidth="5xl"
    >
            ...
            {clients.map((c) => (
              <tr key={c.id} className={`border-t border-slate-800 ${c.id === selfId ? 'bg-brand-600/10' : ''}`}>
                <td className="px-4 py-2 font-mono text-xs">
                  {c.id.slice(0, 8)}
                  {c.id === selfId && <span className="ml-1 text-brand-400">(you)</span>}
                </td>
                ...
                <td className="px-4 py-2 text-right font-mono">
                  {c.lastPingMs == null ? '—' : `${c.lastPingMs} ms`}
                </td>
              </tr>
            ))}
          </tbody>
