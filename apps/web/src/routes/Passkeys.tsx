...
export default function PasskeysPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 pb-16 space-y-6">
      <Link to="/category/identity-payments" className="text-xs text-slate-500 hover:text-slate-300 transition">
        ← Identity &amp; payments
      </Link>

      <div>
        <h1 className="text-2xl font-bold text-slate-100">Passkeys / WebAuthn</h1>
        <p className="mt-1 text-sm text-slate-400">
          Hardware-backed credentials that replace passwords. The private key never leaves the device. Sign-in is biometric or PIN — no secret ever travels over the network.
        </p>
      </div>
