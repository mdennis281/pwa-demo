...
export default function Manifest() {
  ...
  return (
    <div className="p-8 max-w-3xl mx-auto">
      <h1 className="text-3xl font-bold mb-2">Manifest playground</h1>
      <p className="text-slate-400 mb-6">
        Inspect install state and the parsed manifest the browser picked up.
      </p>
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex justify-between border-b border-slate-800 last:border-b-0 py-1">
      ...
      <span className="text-slate-200 font-mono text-right truncate ml-4">{value}</span>
    </div>
  );
}
